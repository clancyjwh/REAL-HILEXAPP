/*
  # Set New Users as Premium by Default

  1. Changes
    - Update handle_new_user() trigger function to set all new users as premium
    - Sets subscription_status to 'active' in users table
    - Sets tier to 'premium' in profiles table
    - All new signups automatically get premium access

  2. Important Notes
    - Everyone who signs up gets premium status immediately
    - No payment or upgrade required
    - Maintains backwards compatibility with existing users
*/

-- Drop existing trigger
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;

-- Update trigger function to set premium by default
