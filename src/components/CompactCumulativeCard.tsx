...
interface CompactCumulativeCardProps {
  value: number;
  onClick: () => void;
}
...
export default function CompactCumulativeCard({ value, onClick }: CompactCumulativeCardProps) {
  const colors = getSignalColors(value);
  const signalPosition = ((value + 10) / 20) * 100;
  const isGold = value >= 9;
