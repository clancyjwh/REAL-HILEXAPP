...
export default function NewsResultsPage() {
  ...
  const extractSources = (text: string) => {
    ...
    // Look for all URLs in the text
    const urlRegex = /(https?:\/\/[^\s\)]+)/g;

    // Split by lines to check if we're in a sources section
    const lines = text.split('\n');
    let inSourcesSection = false;

    for (const line of lines) {
      // Check if we've entered the sources section
      if (line.toLowerCase().includes('sources:') || line.toLowerCase().trim() === 'sources') {
        inSourcesSection = true;
        continue;
      }

      // If we're in the sources section, extract URLs
      if (inSourcesSection) {
        ...
        // If we hit an empty line after sources, we might be done
        // But continue anyway to catch all sources
      }
    }
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-xl p-8">
            <div className="mb-6">
              ...
              <div className="h-3 bg-slate-800 rounded-full overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-blue-500 via-blue-600 to-blue-500 transition-all duration-300 ease-out animate-pulse"
